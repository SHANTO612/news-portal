...
get_writer_news = async (req, res) => {
    ...
    try {
        if (role !== 'writer') {
            return res.status(403).json({ message: 'Access denied. Only writers can access this.' });
        }
...
get_writer_news_statistics = async (req, res) => {
    const { id, role } = req.userInfo;
    try {
        if (role !== 'writer') {
            return res.status(403).json({ message: 'Access denied. Only writers can access this.' });
        ...
        const pendingNews = await newsModel.countDocuments({ writerId: new ObjectId(id), status: 'pending' });
        const activeNews = await newsModel.countDocuments({ writerId: new ObjectId(id), status: 'active' });
...
getWriterNewsStatistics = async (req, res) => {
  try {
    const { id, role } = req.userInfo || {};
    ...
    if (!id || !role) {
      return res.status(400).json({ message: 'User info missing in request' });
    }
    ...
    console.log('Fetching news stats for writer:', id);

    const totalNews = await newsModel.countDocuments({ writerId: id });
    const pendingNews = await newsModel.countDocuments({ writerId: id, status: 'pending' });
    const activeNews = await newsModel.countDocuments({ writerId: id, status: 'active' });
    const deactiveNews = await newsModel.countDocuments({ writerId: id, status: 'deactive' });

    return res.status(200).json({ totalNews, pendingNews, activeNews, deactiveNews });
  } catch (error) {
